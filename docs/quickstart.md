...
PYTHONPATH=src python -m llm_batch_annotate.cli run examples/config/run_config.json --run-id example-single --no-poll-until-terminal
```

Resume the same run later and wait up to 10 polls, every 2 minutes:

```bash
PYTHONPATH=src python -m llm_batch_annotate.cli resume examples/config/run_config.json example-single --poll-interval 2m --max-polls 10
```
...
```bash
export OPEN_AI_KEY="sk-..."
PYTHONPATH=src python -m llm_batch_annotate.cli run examples/config/run_config_2.json --run-id example-grouped --no-poll-until-terminal
```

Resume and continue polling:

```bash
PYTHONPATH=src python -m llm_batch_annotate.cli resume examples/config/run_config_2.json example-grouped --poll-interval 30s --max-polls 20
```
...
- `parsed/flattened_annotations.jsonl` for per-unit outputs.
